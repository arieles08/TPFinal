...
Como feature engineering se aplica dummy a la variable barrios (l3) ya que al tomar solamente la bariable barrio arrojaba demasiado error en la predicción. Por ejemplo que los barrios al ser de Puerto MAdero bajaban de precio en la explicaciòn del BreakDown.
```{r}
...
```{r}
variables_train <- train_data_d %>% select(surface_covered, surface_uncovered, rooms, bathrooms, property_type,l3)
variables_test <- test_data %>% select(surface_covered, surface_uncovered, rooms, bathrooms, property_type,l3,id)
```
...
Primero se selecciona una nueva observación del test_data. En este caso un barrio de Puerto Madero, de 90 metros cuadrados, 55 descubiertos, con 3 habitaciones y 2 baños. 
```{r}
...
```
Score de cada variable en particular fijando el valor de la variable de la nueva observación en todo el data train. Se obtiene el promedio de de las predicciones del train al cambiar los valores de la variable por el de la nueva observacón. Después se obtiene la diferencia entre dicha promedio de predicción y el promedio de las predicciones de llos datos de entrenamiento originales. Se repite para cada variable y se las ordena de forma decreciente, 

l3_PuertoMadero
```{r}
score<-{}
data_train_l3_PurertoMadero<- train_data_d
data_train_l3_PurertoMadero$l3_PuertoMadero<-variable_y$l3_PuertoMadero # Fijo el valor de la variable l3_Puerto madero de la nueva obvervación en el data train
...
data_train_surface_uncovered$surface_uncovered<-variable_y$surface_uncovered # Se coloca el valor de la variable l3_Puerto madero de la nueva obvervación en todos los datos del data train
...
data_train_surface_covered$surface_covered<-variable_y$surface_covered # Se coloca el valor de la variable l3_Puerto madero de la nueva obvervación en todos los datos del data train
...
data_train_bathrooms$bathrooms<-variable_y$bathrooms # Se coloca el valor de la variable l3_Puerto madero de la nueva obvervación en todos los datos del data train
...
data_train_l3_Palermo$l3_Palermo<-variable_y$l3_Palermo # Se coloca el valor de la variable l3_Puerto madero de la nueva obvervación en todos los datos del data train
...
data_train_l3_Belgrano$l3_Belgrano<-variable_y$l3_Belgrano # Se coloca el valor de la variable l3_Puerto madero de la nueva obvervación en todos los datos del data train
...
data_train_property_type$property_type<-variable_y$property_type # Se coloca el valor de la variable l3_Puerto madero de la nueva obvervación en todos los datos del data train
...
data_train_l3_Flores$l3_Flores<-variable_y$l3_Flores # Se coloca el valor de la variable l3_Puerto madero de la nueva obvervación en todos los datos del data train
...
data_train_rooms$rooms<-variable_y$rooms # Se coloca el valor de la variable l3_Puerto madero de la nueva obvervación en todos los datos del data train
...
l3_Almagro
```{r}
data_train_l3_Almagro<- train_data_d
data_train_l3_Almagro$l3_Almagro<-variable_y$l3_Almagro # Se coloca el valor de la variable l3_Puerto madero de la nueva obvervación en todos los datos del data train.
mean(predict(modelo_randomForest, data_train_l3_Almagro))# El promedio de las predicciones al fijar la variable

```

```{r}
delta_l3_Almagro<-abs(mean(predict(modelo_randomForest, data_train_l3_Almagro))-mean(predict(modelo_randomForest, train_data_d)))
delta_l3_Almagro# Score para l3_Almagro
```

```{r}
score[['l3_Almagro']]<-delta_l3_Almagro
```
...
El  segundo paso es obtener las contribuciones de acuerdo al orden ya establcido. Para esto vamos a fijar los valores de las variables de a una a la vez con el valor de la variable en la nueva observación. Obtenemos las predicciones condicionada a la variable y  el promedio de las mismas. Luego tambien se fija la segunda variable y manteniendo fija la anterior tambien. Se obtiene la contribución de forma aditiva. Se repite el procedimiento para todas las variables. Al final vamos a obtener que todas las predicciones del train son iguales a la nueva observación. Por lo tanto converge en el valor de la predicción de la nueva observación.

Contribuciòn de l3_PuertoMadero

```{r}
Contribuciones<-{}
data_train_breakdown<- train_data_d
data_train_breakdown$l3_PuertoMadero<-variable_y$l3_PuertoMadero # Fijo el valor de la variable l3_Puerto madero de la nueva obvervación en el data train
...
data_train_breakdown$surface_uncovered<-variable_y$surface_uncovered # Fijo el valor de la variable l3_Puerto madero de la nueva obvervación en el data train
...
data_train_breakdown$surface_covered<-variable_y$surface_covered # Fijo el valor de la variable l3_Puerto madero de la nueva obvervación en el data train
...
data_train_breakdown$bathrooms<-variable_y$bathrooms # Fijo el valor de la variable l3_Puerto madero de la nueva obvervación en el data train
...
data_train_breakdown$l3_Palermo<-variable_y$l3_Palermo # Fijo el valor de la variable l3_Puerto madero de la nueva obvervación en el data train
...
data_train_breakdown$l3_Belgrano<-variable_y$l3_Belgrano # Fijo el valor de la variable l3_Puerto madero de la nueva obvervación en el data train
...
data_train_breakdown$property_type<-variable_y$property_type # Fijo el valor de la variable l3_Puerto madero de la nueva obvervación en el data train
...
Acc<-mean(predict(modelo_randomForest, data_train_breakdown))
data_train_breakdown$l3_Flores<-variable_y$l3_Flores # Fijo el valor de la variable l3_Puerto madero de la nueva obvervación en el data train
...
data_train_breakdown$l3_Balvanera<-variable_y$l3_Balvanera
Acc<-mean(predict(modelo_randomForest, data_train_breakdown))
data_train_breakdown$rooms<-variable_y$rooms # Fijo el valor de la variable l3_Puerto madero de la nueva obvervación en el data train
mean(predict(modelo_randomForest, data_train_breakdown))
```
...
Acc<-mean(predict(modelo_randomForest, data_train_breakdown))
data_train_breakdown$l3_Almagro<-variable_y$l3_Almagro # Fijo el valor de la variable l3_Puerto madero de la nueva obvervación en el data train
mean(predict(modelo_randomForest, data_train_breakdown))
```

```{r}
delta_l3_Almagro<-mean(predict(modelo_randomForest, data_train_breakdown))-Acc
Acc<-mean(predict(modelo_randomForest, data_train_breakdown))
delta_l3_Almagro# Contribución de l3_almagro 

```

```{r}
Contribuciones[['l3_Almagro']]<-delta_l3_Almagro
```
...
Se puede determinar el orden de las variables manualmente utilizando el parámetro "order". En este ejemplo al cambiar el orden de las variables cambia el valor de la contribuciones. Por lo tanto es aconsejable utilizar ibreakdown que tienen en cuenta las iteraciones entre varibles. 
...
Se aplica de la misma forma que breakdown. Lo único que cambia es el parámetro "typy", el cual debe decir "break_down_interactions".

```{r}
ibd_i_rf <- predict_parts(explainer = explain_rf,
                 new_observation = variable_y,
...
```{r}
plot(ibd_rf, max_features = 7, plot_distributions = TRUE)
```

El orden de las variables lo realiza de forma similar a BreakDown, lo único diferente es que tiene en cuenta una iteración entre variables cuando la score de la iteración es mayor al de las variables por separado y a la suma de las misma. Para que se entienda vamos a comparar los valores del score de las varibles que tienen iteración que es el caso de superficie cubierta y baño. 
...
data_train_surface_covered$surface_covered<-variable_y$surface_covered # Se coloca el valor de la variable l3_Puerto madero de la nueva obvervación en todos los datos del data train
delta_surface_covered<-abs(mean(predict(modelo_randomForest, data_train_surface_covered))-mean(predict(modelo_randomForest, train_data_d)))
delta_surface_covered# Score para surface_covered
```
...
delta_bathrooms# Score para bathrooms
```
Suma de score de superficie cubierta y score de baño
```{r}
suma_surface_bathrooms<-delta_bathrooms+delta_surface_covered
suma_surface_bathrooms
```

Score de iteración de superficie cubierta y baño
```{r}
data_train_surface_bathrooms<- train_data_d
data_train_surface_bathrooms$surface_covered<-variable_y$surface_covered
data_train_surface_bathrooms$bathrooms<-variable_y$bathrooms # Se coloca el valor de la variable l3_Puerto madero de la nueva obvervación en todos los datos del data train
delta_surface_bathrooms<-abs(mean(predict(modelo_randomForest, data_train_surface_bathrooms))-mean(predict(modelo_randomForest, train_data_d)))
delta_surface_bathrooms# iteración de superficie cubierta y baño
```
Como es más alto el valor de la iteración que cada variable individual y su suma. Solamamente se representa la iteración entre superficie cubierta y baño. Individualmente n se representan. 
...
delta_surface_uncovered<-abs(mean(predict(modelo_randomForest, data_train_breakdown)-Acc))
Acc<-mean(predict(modelo_randomForest, data_train_breakdown))
delta_surface_uncovered# Score surface_uncovered
...
Contribución de surface_covered:bathrooms
```{r}

data_train_breakdown$surface_covered<-variable_y$surface_covered 
data_train_breakdown$bathrooms<-variable_y$bathrooms# Fijo el valor de la variable l3_Puerto madero de la nueva obvervación en el data train
mean(predict(modelo_randomForest, data_train_breakdown))
```

```{r}
delta_surface_covered_bathrooms<-abs(mean(predict(modelo_randomForest, data_train_breakdown)-Acc))
Acc<-mean(predict(modelo_randomForest, data_train_breakdown))
delta_surface_covered_bathrooms# Contribución de surface_covered:bathrooms
```


```{r}
Contribuciones[['surface_covered_bathrooms']]<-delta_surface_covered_bathrooms
```
...
delta_l3_Belgrano<-abs(mean(predict(modelo_randomForest, data_train_breakdown)-Acc))
Acc<-mean(predict(modelo_randomForest, data_train_breakdown))
delta_l3_Belgrano# Contribución l3_Belgrano
...
Acc<-mean(predict(modelo_randomForest, data_train_breakdown))
data_train_breakdown$l3_Flores<-variable_y$l3_Flores # Fijo el valor de la variable l3_Puerto madero de la nueva obvervación en el data train
mean(predict(modelo_randomForest, data_train_breakdown))# El promedio de las predicciones al decir que todas las propiedades del train son de puerto madero, con superficie descubierta de 55 metros cuadrados, con superficie cubierta de 
```
...
data_train_breakdown$l3_VillaDevoto<-variable_y$l3_VillaDevoto
Acc<-mean(predict(modelo_randomForest, data_train_breakdown))
data_train_breakdown$l3_Balvanera<-variable_y$l3_Balvanera # Fijo el valor de la variable l3_Puerto madero de la nueva obvervación en el data train
mean(predict(modelo_randomForest, data_train_breakdown))
```
...
Acc<-mean(predict(modelo_randomForest, data_train_breakdown))
delta_l3_Balvanera# Contribución de l3_Balvanera 

```

```{r}
Contribuciones[['l3_balvanera']]<-delta_l3_Balvanera
```
